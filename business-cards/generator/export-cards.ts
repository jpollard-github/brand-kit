...
function buildWorkFrontHtml(copy: CopySet, assets: AssetBundle) {
  ...
  return `
    <article class="card" data-card-id="work-with-me-front">
      ...
      <div class="box" style="${boxStyle(boxes.name)}">
        <div class="eyebrow">Work With Me</div>
...
function buildArcadeFrontHtml(copy: CopySet, assets: AssetBundle) {
  ...
  return `
    <article class="card arcade-front" data-card-id="arcadeghosts-front">
      ...
      <div class="box" style="${boxStyle(boxes.title)}">
        <div class="eyebrow" style="color: var(--cyan)">ArcadeGhosts</div>
...
async function loadCopy(): Promise<CopySet> {
  return {
    workFront: await readLines(path.join(rootDir, "work-with-me", "front-copy.txt")),
    workBack: await readLines(path.join(rootDir, "work-with-me", "back-copy.txt")),
    arcadeFront: await readLines(path.join(rootDir, "arcadeghosts", "front-copy.txt")),
    arcadeBack: await readLines(path.join(rootDir, "arcadeghosts", "back-copy.txt")),
  };
}

async function ensureDirectories() {
  await fs.mkdir(path.join(rootDir, "work-with-me", "exports"), { recursive: true });
  await fs.mkdir(path.join(rootDir, "arcadeghosts", "exports"), { recursive: true });
  await fs.mkdir(generatorDir, { recursive: true });
}

async function loadAssets(): Promise<AssetBundle> {
  const logoPath = path.join(sharedDir, "logo.png");
  const workQrPath = path.join(sharedDir, "qr-work-with-me.svg");
  const arcadeQrPath = path.join(sharedDir, "qr-arcadeghosts.svg");

  await Promise.all([
    requireAsset(logoPath, "logo.png"),
    requireAsset(workQrPath, "qr-work-with-me.svg"),
    requireAsset(arcadeQrPath, "qr-arcadeghosts.svg"),
  ]);

  const [logoDataUrl, workQrDataUrl, arcadeQrDataUrl] = await Promise.all([
    readAssetAsDataUrl(logoPath),
    readAssetAsDataUrl(workQrPath),
    readAssetAsDataUrl(arcadeQrPath),
  ]);
...
async function buildCards(): Promise<CardExport[]> {
  ...
  return [
    {
      ...
      exportPath: path.join(rootDir, "work-with-me", "exports", "front-final.png"),
      html: buildWorkFrontHtml(copy, assets),
    },
    {
      id: "work-with-me-back",
      title: "Work With Me Back",
      exportPath: path.join(rootDir, "work-with-me", "exports", "back-final.png"),
      html: buildWorkBackHtml(copy, assets),
    },
    {
      id: "arcadeghosts-front",
      title: "ArcadeGhosts Front",
      exportPath: path.join(rootDir, "arcadeghosts", "exports", "front-final.png"),
      html: buildArcadeFrontHtml(copy, assets),
    },
    {
      id: "arcadeghosts-back",
      title: "ArcadeGhosts Back",
      exportPath: path.join(rootDir, "arcadeghosts", "exports", "back-final.png"),
...
async function exportCardVariant(pageHtml: string, exportPath: string, guides: boolean, pdf: boolean) {
...
async function exportCards(cards: CardExport[], options: ExportOptions) {
  ...
  for (const card of cards) {
    ...
    if (cleanDimensions.width !== exportSize.width || cleanDimensions.height !== exportSize.height) {
      throw new Error(`${card.title} clean PNG failed validation: expected ${exportSize.width}x${exportSize.height}, got ${cleanDimensions.width}x${cleanDimensions.height}`);
    }
    ...
    if (options.guides) {
      ...
      await exportCardVariant(exportDocument(card), guidePngPath, true, options.pdf);

      const guideDimensions = await verifyPng(guidePngPath);
      if (guideDimensions.width !== exportSize.width || guideDimensions.height !== exportSize.height) {
        throw new Error(`${card.title} guide PNG failed validation: expected ${exportSize.width}x${exportSize.height}, got ${guideDimensions.width}x${guideDimensions.height}`);
      }
...
async function logOutputs(cards: CardExport[], options: ExportOptions) {
  for (const card of cards) {
    const cleanPngPath = withVariantSuffix(card.exportPath, false);
    const cleanDimensions = await verifyPng(cleanPngPath);
    console.log(`${card.title}: ${path.relative(rootDir, cleanPngPath)} ${cleanDimensions.width}x${cleanDimensions.height}`);

    if (options.guides) {
      const guidePngPath = withVariantSuffix(card.exportPath, true);
      const guideDimensions = await verifyPng(guidePngPath);
      console.log(`${card.title} Guides: ${path.relative(rootDir, guidePngPath)} ${guideDimensions.width}x${guideDimensions.height}`);

      if (options.pdf) {
        const guidePdfPath = guidePngPath.replace(/\.png$/, ".pdf");
        const bytes = await verifyFileExists(guidePdfPath);
        console.log(`${card.title} Guides PDF: ${path.relative(rootDir, guidePdfPath)} ${bytes} bytes`);
      }
    } else if (options.pdf) {
      const cleanPdfPath = cleanPngPath.replace(/\.png$/, ".pdf");
      const bytes = await verifyFileExists(cleanPdfPath);
      console.log(`${card.title} PDF: ${path.relative(rootDir, cleanPdfPath)} ${bytes} bytes`);
    }
  }
}
...
async function main() {
  ...
  console.log(`Preview written to ${path.relative(process.cwd(), path.join(generatorDir, "cards.html"))}`);

  if (options.guides) {
    console.log("Guides mode enabled: trim/safe area overlays were included in proof exports only.");
  }
}
