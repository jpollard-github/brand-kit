...
async function loadCopy(brandId: string): Promise<CopySet> {
  ...
  const workFrontPath = path.join(
    brandRootDir,
    "copy",
    "work-with-me",
    "front-copy.txt",
  );
  const workBackPath = path.join(
    brandRootDir,
    "copy",
    "work-with-me",
    "back-copy.txt",
...
async function loadAssets(brandId: string): Promise<AssetBundle> {
  ...
  return {
    paths: {
      ...
      workQrPath: resolvedWorkQrPath,
      arcadeQrPath: resolvedArcadeQrPath,
    },
    logoDataUrl,
    workQrDataUrl,
    arcadeQrDataUrl,
...
async function buildCards(brandConfig: BrandConfig): Promise<CardExport[]> {
  ...
  const verificationReport = await verifyBusinessCardSources(
    brandConfig,
    copy,
    assets.paths,
  );
  logVerificationReport("Brand source verification", verificationReport);
  assertVerificationPassed(verificationReport);
