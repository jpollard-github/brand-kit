...
<svg xmlns="http://www.w3.org/2000/svg" width="2400" height="3000" viewBox="0 0 2400 3000">
  <rect width="2400" height="3000" rx="220" fill="${brand.palette.background}" />
  <rect x="110" y="110" width="2180" height="2780" rx="180" fill="${brand.palette.backgroundSoft}" stroke="${brand.palette.border}" stroke-width="10" />
  <path d="M520 380 C760 220, 1640 220, 1880 380" fill="none" stroke="${brand.palette.teal}" stroke-opacity="0.2" stroke-width="12" />
  <image href="${safeLogoDataUrl}" x="420" y="720" width="520" height="520" preserveAspectRatio="xMidYMid meet" />
  <text x="1120" y="980" fill="${brand.palette.text}" font-family="${fontStack}" font-size="122" font-weight="700">${displayName}</text>
  <text x="1120" y="1145" fill="${brand.palette.textMuted}" font-family="${fontStack}" font-size="64" font-weight="600" letter-spacing="10">LEFT-CHEST FRONT</text>
  <text x="1200" y="2630" text-anchor="middle" fill="${brand.palette.amber}" font-family="${fontStack}" font-size="74" font-weight="560">wearable first</text>
</svg>`;

  const backSvg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="2400" height="3000" viewBox="0 0 2400 3000">
  <rect width="2400" height="3000" rx="220" fill="${brand.palette.background}" />
  <rect x="110" y="110" width="2180" height="2780" rx="180" fill="${brand.palette.backgroundDeep}" stroke="${brand.palette.border}" stroke-width="10" />
  <path d="M430 710 C710 430, 1690 430, 1970 710" fill="none" stroke="${brand.palette.pink}" stroke-opacity="0.24" stroke-width="14" />
  <path d="M560 840 C810 640, 1590 640, 1840 840" fill="none" stroke="${brand.palette.cyan}" stroke-opacity="0.18" stroke-width="10" />
  <text x="1200" y="1220" text-anchor="middle" fill="${brand.palette.text}" font-family="${fontStack}" font-size="166" font-weight="720">${displayName}</text>
  <text x="1200" y="1460" text-anchor="middle" fill="${brand.palette.amber}" font-family="${fontStack}" font-size="84" font-weight="560">${backPhrase}</text>
  <text x="1200" y="1720" text-anchor="middle" fill="${brand.palette.textMuted}" font-family="${fontStack}" font-size="60" font-weight="600" letter-spacing="12">UPPER-BACK GRAPHIC</text>
  <circle cx="1200" cy="2050" r="210" fill="none" stroke="${brand.palette.teal}" stroke-opacity="0.24" stroke-width="12" />
  <circle cx="1200" cy="2050" r="145" fill="none" stroke="${brand.palette.amber}" stroke-opacity="0.18" stroke-width="8" />
</svg>`;
